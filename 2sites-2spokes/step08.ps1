...
# - store the configuration of Virtual WAN in the storage account
# - get the configuration of Virtual WAN from the storage account
# - parse the configuration of Virtual WAN 
# - create the configuration of CSR1 and CSR2
# - copy the configuration of CSR1 and CSR2 in the script directory
...
$vpnSite1Name       = "site1"              # name of the site1
$vpnSite2Name       = "site2"              # name of the site2
...
Get-AzVirtualWanVpnConfiguration -VirtualWan $virtualWan -StorageSasUrl $sasURI -VpnSite $vpnSitesForConfig
...
for ($j=0;$j -lt $vWANConfig.vpnSiteConfiguration.Count; $j++ ) {
    ...
    if ($vWANConfig[$j].vpnSiteConfiguration.Name -eq "site1")
    {
      $i=$j
      write-host "value i:"$i -ForegroundColor Yellow
    }
}
...
function generate_csrConfig{
 Param(
     ...
     [Parameter(Mandatory=$true)] [System.String]$ip_loopback,          ### ip address lookback interface inclusive of mask
     ...
     [Parameter(Mandatory=$true)] [System.String]$ip_Tunnel0,           ### ip address of the tunnel0 interface of CSR, inclusive of mask
     [Parameter(Mandatory=$true)] [System.String]$ip_Tunnel1,           ### ip address of the tunnel1 interface of CSR, inclusive of mask
...
write-host "remote site1-ip address lookback interface: "$ip_loopback -ForegroundColor Cyan
...
write-host "CSR-ip address of the tunnel0 interface...: "$ip_Tunnel0   -ForegroundColor Green
write-host "CSR-ip address of the tunnel1 interface...: "$ip_Tunnel1 -ForegroundColor Green
...
{ 
   ...
   switch ($k)
   {
    "site1"  { 
       Write-Host ""
       write-host "-----------------------site1" -ForegroundColor Cyan
       $index = [Array]::FindIndex( $vWANConfig, [System.Predicate[pscustomobject]]{ $args[0].vpnSiteConfiguration.Name -eq "site1" } ) ### get the index of array related to "site1"
       $localASN=$vWANConfig.vpnSiteConfiguration[$index].BgpSetting.Asn                                                    ### BGP ASN assigned to the remote site 
       $ip_loopback=$vWANConfig.vpnSiteConfiguration[$index].BgpSetting.BgpPeeringAddress+" 255.255.255.255"                ### ip address lookback interface inclusive of mask
       ...
       $ip_Tunnel0="172.16.0.1 255.255.255.255"     ### ip address of the tunnel0 interface of CSR, inclusive of mask
       $ip_Tunnel1="172.16.0.2 255.255.255.255"     ### ip address of the tunnel1 interface of CSR, inclusive of mask
    ...
    "site2" { 
       Write-Host ""
       write-host "-----------------------site2" -ForegroundColor Cyan
       $index = [Array]::FindIndex( $vWANConfig, [System.Predicate[pscustomobject]]{ $args[0].vpnSiteConfiguration.Name -eq "site2" } ) ### get the index of array related to "site1"
       $localASN=$vWANConfig.vpnSiteConfiguration[$index].BgpSetting.Asn                                                    ### BGP ASN assigned to the remote site 
       $ip_loopback=$vWANConfig.vpnSiteConfiguration[$index].BgpSetting.BgpPeeringAddress+" 255.255.255.255"                ### ip address lookback interface inclusive of mask
       ...
       $ip_Tunnel0="172.16.0.3 255.255.255.255"     ### ip address of the tunnel0 interface of CSR, inclusive of mask
       $ip_Tunnel1="172.16.0.4 255.255.255.255"     ### ip address of the tunnel1 interface of CSR, inclusive of mask
