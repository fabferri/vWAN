...
##  In each site:
##  - create a resource group to deploy the site
##  - create an Azure VNet with to two subnets: subnet1, subnet2 
##  - in the VNet create one Cisco CSR with two NICs: one in subnet1 and one in subnet2
##  - create a VM in subnet2
##  - set NSG to accept SSH connections
##  - associate the NSG to the subnet2
...
##################################################################
###################### Variables #################################
$subscriptionName= "AzureDemo3"   # Name Azure subscription
...
$siteArray = @(
  @{
       ...
       location           = "westeurope"    
       vnetName           = "site1-vnet"
       vnetPrefix         = @("10.3.10.0/24","10.3.11.0/24")
       subnet1Prefix      = "10.3.10.0/24"
       subnet2Prefix      = "10.3.11.0/24"
       subnet1Name        = "subnet1"   
       subnet2Name        = "subnet2"
       ...
       csr_privateIP1     = "10.3.10.5"
       csr_privateIP2     = "10.3.11.5"
  ...
  @{
       ...
       location           = "westeurope"    
       vnetName           = "site2-vnet"
       vnetPrefix         = @("10.3.20.0/24","10.3.21.0/24")
       subnet1Prefix      = "10.3.20.0/24"
       subnet2Prefix      = "10.3.21.0/24"
       subnet1Name        = "subnet1"   
       subnet2Name        = "subnet2"
       csr_adminName      = $adminUsername
       csr_adminPwd       = $adminPassword
       ...
       csr_privateIP1     = "10.3.20.5"
       csr_privateIP2     = "10.3.21.5"
       csr_publisherName  = "cisco"            
       csr_offerName      = "cisco-csr-1000v"  
       csr_skuName        = "csr-azure-byol"   
       csr_version        = "latest"
...
try {
   ...
   write-host 'csr-PubIP Address:'$csr_publicIP.Name'already exists. skip creation' -foregroundcolor Green -backgroundcolor Black
 } catch {
 $csr_publicIP = New-AzPublicIpAddress `
           ...
           -Force 
  write-host 'csr-PubIP Address:'$csr_publicIP.Name'has been created.' -foreground Yellow
}
...
try {
  ...
  write-host 'csr-NIC1         :'$csr_nic1.Name'already exists. skip creation' -foregroundcolor Green -backgroundcolor Black
} catch {
  $csr_nic1 = New-AzNetworkInterface `
                ...
                -EnableIPForwarding `
                -Force -Verbose 
 write-host 'csr-NIC1         :'$csr_nic1.Name'has been created.' -foreground Yellow
}
...
try {
  $csr_nic2 = Get-AzNetworkInterface `
                -Name $csr_nic2Name `
                -ResourceGroupName $rgName -ErrorAction Stop
  write-host 'csr-NIC2         :'$csr_nic2.Name'already exists. skip creation' -foregroundcolor Green -backgroundcolor Black
} catch {
  $csr_nic2 = New-AzNetworkInterface `
                ...
                -EnableIPForwarding `
                -Force -Verbose 
  write-host 'csr-NIC2         :'$csr_nic2.Name'has been created.' -foreground Yellow
}
...
try {
  ...
  write-host 'csr..............:'$csr.Name'already exists... skipping' -foregroundcolor Green -backgroundcolor Black
} catch {
  ...
  $csr=New-AzVM -VM $csr_Config `
            ...
            -Location $location `
            -verbose
  $csr = Get-AzVM -Name $csr_Name -ResourceGroupName $rgName
  write-host 'csr..............:'$csr.Name'has been created.' -foregroundcolor Yellow
}
...
try {
   ...
   write-host 'vm- PubIP Address:'$vm_publicIP.Name'already exists. skip creation' -foregroundcolor Green -backgroundcolor Black
 } catch {
   $vm_publicIP = New-AzPublicIpAddress `
           ...
           -Force
   write-host  'vm- PubIP Address:'$vm_publicIP.Name'has been created.' -foreground Yellow
}
...
try {
  ...
  $vm_nic = get-AzNetworkInterface `
                -Name $vm_nicName `
                -ResourceGroupName $rgName -ErrorAction Stop
  write-host 'vm-NIC           :'$vm_nic1.Name'already exists. skip creation' -foregroundcolor Green -backgroundcolor Black
} catch {
  $vm_nic = New-AzNetworkInterface `
                ...
                -Force -Verbose
  write-host 'vm-NIC           :'$vm_nic1.Name'has been created.' -foreground Yellow
...
try {
  ...
  write-host 'vm...............:'$vm.Name'already exists... skipping' -foregroundcolor Green -backgroundcolor Black
} catch {
  ...
  $vm=New-AzVM -VM $vm_Config `
            ...
            -Location $location `
            -verbose
  $vm = Get-AzVM -Name $vm_Name -ResourceGroupName $rgName
  write-host 'vm:'$vm.Name'has been created.' -foregroundcolor Yellow
}
...
try {$rt = Get-AzRouteTable -Name $rtName -ResourceGroupName $rgName -ErrorAction Stop
     Write-Host "  $rtName route table exists, skipping" -foregroundcolor  Green -backgroundcolor Black
} catch {
